...
namespace collections_question_2 {
    ...
    class Program {
        static void Main(string[] args) {
            ...
            number.Sort();
            min_three.Add(number[0]);
            min_three.Add(number[1]);
            min_three.Add(number[2]);

            number.Reverse();
            max_three.Add(number[0]);
            max_three.Add(number[1]);
            max_three.Add(number[2]);

            double sumMin = 0;
            foreach (int i in min_three) {
                sumMin = sumMin + i;
            }
